...
const { FriendLink, Image } = require('@models');
...
exports.fetchPreview = async (req, res) => {
  ...
  // 新图记录 + 回填引用；旧图因 preview_image_id 被覆盖失去引用，由 GC 对账回收
  ...
  const updateData = { preview_image: saved.storagePath, preview_image_id: record.image_id };
